const {
  randomTokenByCrypto,
  hashTokenByCrypto,
} = require("../middlewares/cryptoToken");
const {
...
const register = async (req, res) => {
  try {
    ...
    if (!response?.confirm)
      throw new Error("Account already exists or unconfirmed!");
    ...
    sendMail({
      ...
      html: `<div >
            <p >Để thay đổi mật khẩu cho tài khoản ${email} của bạn vui lòng bấm vào link bên dưới, đường link có hiệu lực trong vòng 5 phút
             ...
             <a href='${
               process.env.URL_CLIENT + "/reset_password/" + token
             }' >Click vào đây!</a>
            </div>`,
...
const resetPassword = async (req, res) => {
  try {
    ...
    res.status(200).json({
      ...
      message: user
        ? "Changer password successfully"
        : "Changer password failed",
      user,
    });
  } catch (error) {}
};
  ...
  try {
    ...
    if (!users) {
      ...
      user = await User.findOneAndUpdate(
        { email },
        { firstName: given_name, lastName: family_name, avatar_url: picture },
        { new: true }
      );
    }
    if (!user) {
